...
HTTP through `src/features/global-news/lib/newsService.ts`.
...
Code inspected: backend `src/features/global-news/providers/types.ts`, `src/features/global-news/ingest/scheduler.ts`,
`src/util/rateLimiter.ts`, `src/db/pool.ts`, `src/api/server.ts` and
`src/features/global-news/api/routes/operations.ts`; frontend `README.md`, `src/features/global-news/lib/newsService.ts`,
`src/features/global-news/components/GlobalNewsPage.tsx`, `src/features/companies/lib/useLiveQuote.ts`,
`src/features/research/lib/pipeline.ts` and `src/lib/claude.ts`.
