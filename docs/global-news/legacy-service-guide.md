> Historical guide for the original news service. These notes preserve the earlier setup and measured results. Start with the repository README for the current layout and commands.
...
| --- | --- |
| **[docs/deployment/vercel.md](../../docs/deployment/vercel.md)** | Deploy the API on Vercel, fix the missing `public` error, and connect a separate ingestion worker |
| **[docs/deployment/persistent.md](../../docs/deployment/persistent.md)** | Persistent production service, database bootstrap, required settings and recovery |
| **[docs/global-news/provider-comparison.md](../../docs/global-news/provider-comparison.md)** | News provider options priced and **measured against this catalogue**, and the primary + fallback recommendation |
| **[docs/global-news/api.md](../../docs/global-news/api.md)** | Full API reference. Machine-readable spec: [docs/api/openapi.json](../../docs/api/openapi.json), also live at `/docs` |
| **[docs/global-news/operations.md](../../docs/global-news/operations.md)** | Where this breaks first when it runs unattended for months, and what I deliberately did not build |
| **[docs/global-news/report.md](../../docs/global-news/report.md)** | **Full project report** — coverage, accuracy, latency, freshness, sources, cost, and the free-vs-paid recommendation |
...
with measurements in [docs/global-news/provider-comparison.md](../../docs/global-news/provider-comparison.md).
...
```bash
npx tsx scripts/global-news/resolve-links.ts --dry-run   # report only
npx tsx scripts/global-news/resolve-links.ts             # rewrite, merging duplicates
```
...
ingest**, not stored and hidden — see [docs/global-news/report.md](../../docs/global-news/report.md) §7b.
...
**Result:** 1,512 of 1,515 companies resolved (**99.7%**), 2,023 listings,
153 depositary receipts identified.
...
**1,482 companies had a US listing and went to Finnhub; the rest fell through
...
[docs/global-news/report.md](../../docs/global-news/report.md) §6a.
...
**Layer boundaries.** Shared catalogue code remains under `src/catalogue/`;
the original news pipeline lives under `src/features/global-news/`. Each layer
owns one job and talks to the next only through types:

| Layer | Responsibility |
| --- | --- |
| `catalogue/` | Parse and audit the supplier file. Never corrects it. |
| `features/global-news/resolution/` | Ticker → security → the full set of listings. |
| `features/global-news/providers/` | One adapter interface; each source behind it. |
| `features/global-news/ingest/` | Orchestration, canonicalisation, deduplication, run isolation. |
| `features/global-news/observability/` | Run and per-company outcome recording. |
| `features/global-news/api/` | HTTP contract. Reads through its own query layer. |
...
steer the pipeline. See [docs/global-news/report.md](../../docs/global-news/report.md) §7b.
...
Implement `NewsProvider` ([src/features/global-news/providers/types.ts](../../src/features/global-news/providers/types.ts)),
register it in [src/features/global-news/providers/registry.ts](../../src/features/global-news/providers/registry.ts), and name
...
| Comparison write-up | [docs/global-news/provider-comparison.md](../../docs/global-news/provider-comparison.md) |
...
| How it was produced | [docs/global-news/provider-comparison.md](../../docs/global-news/provider-comparison.md) §1, and `src/features/global-news/resolution/` |
| Which companies are **not** covered, and why | `npm run export:gaps` → [data/coverage-gaps.csv](../../data/coverage-gaps.csv); analysed in [docs/global-news/report.md](../../docs/global-news/report.md) |
| Think past the happy path | [docs/global-news/operations.md](../../docs/global-news/operations.md) |
...
| `npm run export:openapi` | Regenerate `docs/api/openapi.json` from the routes |
| `npm run metrics` | Regenerate every figure quoted in docs/global-news/report.md from the live database, including the connectivity breakdown |
| `npm test` | Unit tests |
